...
// Origin allow-list. In PRODUCTION only the official GitHub Pages copy may
// talk to this server, so a cloned site on another domain — or someone who
// opens index.html locally / hosts their own copy — is rejected. Localhost
// and other dev origins are blocked unless ALLOW_DEV_ORIGINS=true is set
// (do that only on your dev machine, never on the live Render service).
const PROD_ORIGINS = [
  'https://zdstudios.github.io',     // official GitHub Pages host
];
const DEV_ORIGINS = [
  'http://localhost:3000',
  'http://localhost:5500',
...
const ALLOWED_ORIGINS = process.env.ALLOWED_ORIGINS
  ...
  : (ALLOW_DEV_ORIGINS ? [...PROD_ORIGINS, ...DEV_ORIGINS] : [...PROD_ORIGINS]);
...
function isAllowedOrigin(origin) {
  // No Origin header = not a real browser tab on an allowed site (curl, a
  // bot, a custom socket client, a file:// page) -> reject when strict.
...
// Super-admin: the code is validated SERVER-SIDE only and we never keep it
// in plaintext. We store ONLY a salted SHA-256 hash, so even someone who
// reads the source or the env vars can't recover the actual PIN, and the
// check uses a timing-safe comparison. To change the PIN on Render set an
// ADMIN_CODE env var (it gets hashed at boot), or supply a precomputed
// ADMIN_CODE_HASH directly. Optionally override ADMIN_SALT too.
const ADMIN_SALT = process.env.ADMIN_SALT || 'zd-car-sim::v1::admin-salt';
function hashAdmin(code) {
  return crypto.createHash('sha256').update(ADMIN_SALT + '::' + String(code)).digest('hex');
}
const ADMIN_CODE_HASH = (process.env.ADMIN_CODE_HASH
  ? process.env.ADMIN_CODE_HASH.trim().toLowerCase()
  : hashAdmin(process.env.ADMIN_CODE || '8089'));
function adminCodeMatches(code) {
  if (!code) return false;
  const a = Buffer.from(hashAdmin(code), 'utf8');
  const b = Buffer.from(ADMIN_CODE_HASH, 'utf8');
  if (a.length !== b.length) return false;
  return crypto.timingSafeEqual(a, b);
}
const superAdmins = new Set(); // socket ids that entered the correct code

// Brute-force guard for admin login: a short PIN is guessable, so we lock
// out an IP after too many wrong tries for a cooldown window.
...
function clientIp(socket) {
  const xf = socket.handshake.headers['x-forwarded-for']; // Render sits behind a proxy
  if (xf) return String(xf).split(',')[0].trim();
  return socket.handshake.address || 'unknown';
}

// Per-IP connection-rate limiter — stops someone hammering the server with
// reconnects to flood it.
const connRate = new Map(); // ip -> [timestamps]
const CONN_MAX = 30, CONN_WINDOW = 60 * 1000;
function connectionAllowed(ip) {
  const now = Date.now();
  let arr = connRate.get(ip);
  if (!arr) { arr = []; connRate.set(ip, arr); }
  while (arr.length && arr[0] < now - CONN_WINDOW) arr.shift();
  if (arr.length >= CONN_MAX) return false;
  arr.push(now);
  return true;
}

// Per-socket, per-event rate limiter — stops a malicious client spamming
// chat / combat / admin events.
function rateOk(socket, key, max, windowMs) {
  if (!socket._rl) socket._rl = {};
  const now = Date.now();
  let b = socket._rl[key];
  if (!b || now - b.start >= windowMs) { b = { start: now, n: 0 }; socket._rl[key] = b; }
  b.n++;
  return b.n <= max;
}

// Periodically prune the abuse maps so they don't grow unbounded.
setInterval(() => {
  const now = Date.now();
  for (const [ip, rec] of adminFails) if (rec.until && rec.until < now) adminFails.delete(ip);
  for (const [ip, arr] of connRate) {
    while (arr.length && arr[0] < now - CONN_WINDOW) arr.shift();
    if (arr.length === 0) connRate.delete(ip);
  }
}, 5 * 60 * 1000);
...
io.on('connection', (socket) => {
  ...
  // Per-IP connection flood guard.
  if (!connectionAllowed(ip)) {
    console.log('[conn REJECTED flood]', ip);
    socket.disconnect(true);
    return;
  }

  // Hard origin check — sockets bypass CORS, so verify here too. Only the
  // official GitHub Pages copy is allowed (plus dev origins if enabled).
  ...
  // Lightweight client-app tag — blocks generic socket.io scanners/clients
  // that connect without the handshake our real client sends.
  const auth = socket.handshake.auth || {};
  if (STRICT_CORS && auth.app !== 'zd-car-sim') {
    console.log('[conn REJECTED bad app tag]', socket.id);
    try { socket.emit('connectionRejected', { reason: 'unauthorized_client' }); } catch {}
    socket.disconnect(true);
    return;
  }
  ...
  socket.on('adminLogin', (d, ack) => {
    ...
    if (rec && rec.until > now) {
      if (ack) ack({ ok: false, error: 'Too many attempts — locked out, try again later.' });
      return;
    }
    ...
    if (adminCodeMatches(code)) {
      ...
      r.n = (r.n || 0) + 1;
      if (r.n >= ADMIN_MAX_FAILS) { r.until = now + ADMIN_LOCK_MS; r.n = 0; }
      adminFails.set(ip, r);
      console.log('[admin] DENIED', socket.id, ip);
  ...
  socket.on('fire', (d) => {
    if (!rateOk(socket, 'fire', 25, 1000)) return; // cap tracer spam
    const lobby = currentLobby();
    if (!lobby || !d) return;
  ...
  socket.on('hitPlayer', (d) => {
    if (!rateOk(socket, 'hit', 40, 1000)) return;
    const lobby = currentLobby();
  ...
  socket.on('chatMessage', (data) => {
    if (!rateOk(socket, 'chat', 8, 5000)) return; // anti-spam: 8 msgs / 5s
    const lobby = currentLobby();
    if (!lobby) return;
    const p = lobby.players.get(socket.id);
    if (!p) return;
  ...
  socket.on('adminCommand', (d) => {
    if (!rateOk(socket, 'admincmd', 20, 1000)) return;
    const lobby = currentLobby();
    if (!lobby || !d) return;
